...
Module Order_Export
    ...
    Sub Main()
        ...
        Dim exportFileName As String = "SERVU-" & fileTime.ToString("yyyyMMddHHmm") & ".txt"
        ...
        Try
            ...
            'processing
            Dim orderSQLStatement As String = "SELECT a.entity_id, a.coupon_code, a.base_discount_amount, " & _
                ...
                "b.cc_last4, b.cc_exp_month, b.cc_exp_year, b.cc_type, b.cc_number_enc, b.method, c.sku, c.qty_ordered, " & _
                "c.price, c.name, a.created_at, a.increment_id, a.customer_note, c.parent_item_id FROM sales_flat_order AS a LEFT OUTER JOIN sales_flat_order_payment AS b on a.entity_id= b.entity_id " & _
                "LEFT OUTER JOIN sales_flat_order_item AS c ON a.entity_id = c.order_id WHERE a.status = 'pending'"
                ...
                If currentOrder <> previousOrder Then
                    ...
                    Try
                        ...
                        Dim billingAndShippingSQLStatment As String = "SELECT a.parent_id, a.fax, a.region, " & _
                                ...
                                "a.country_id, a.address_type, a.company FROM sales_flat_order_address AS a " & _
                        ...
                        Dim expMonth, expYear, exp, couponValue, couponCode, creditCardField, shippingDescription As String
                        ...
                        'decrypt the credit card number field if there is a credit card #
                        ...
                        If expMonth <> "" And expMonth.Length < 3 And creditCardField <> "" Then
                        ...
                        exportOutput &= CType(createField(exp, 14), String)
                        'Contains Defered Billing(1) and  AUTH_DATE(8)
                        exportOutput &= CType(createField(orderResultReader, "customer_email", "", 44), String)
            ...
            If hasCoupon Then
                exportOutput &= CType(buildFakeLineItem(), String) & vbCrLf
            End If
        ...
        If successfulOrders.Count <> 0 Then
            sendExportResults("Order Export: " & exportFileName, exportFileName & " exported successfully.", exportFileName, exportOutput)
            updateOrders()
        Else
            sendExportResults("Order Export: No orders", exportFileName & " There were no orders to export.", exportFileName, "")
        End If
    ...
    Private Sub updateOrders()
        ...
        Dim whereClause, whereGridClause, sqlStatement As String
        ...
        '"JOIN sales_flat_order_status_history ON sales_flat_order_status_history.entity_id " & _
        '"= sales_flat_order_grid.entity_id SET sales_flat_order.state = 'processing', sales_flat_order.status = 'fulfilling', " & _
        '"sales_flat_order_grid.status = 'fulfilling', sales_flat_order_status_history.status = 'fulfilling' " & _
        '"WHERE sales_flat_order.entity_id = " & whereClause
        'processing, fulfilling
        sqlStatement = "UPDATE sales_flat_order SET sales_flat_order.state = 'processing', sales_flat_order.status = 'processing' WHERE sales_flat_order.entity_id = " & whereClause & ";"
        'fulfilling
    ...
    Private Function buildFakeLineItem() As String
